<template>
  <div class="wrapper">
    ...
    <van-form :show-error="false" @submit="onSubmit">
      ...
      <van-cell v-if="showDetail" class="details" :border="false" is-link value="详情">
        <template #title>
      ...
      <template v-for="(item,index) in watchTime">
        ...
        >
          <template #label>
            <span :style="{'opacity': index==0?1:0}">看车时间</span>
          </template>
          <template #right-icon>
            <div class="actionBtns">
              <svg-icon v-if="index>0" class="icon" icon-class="reduce" @click.stop="reduceHandle(index)" />
              <svg-icon v-if="index<2 && item.show" class="icon" icon-class="add" @click.stop="addHandle(index)" />
            </div>
          </template>
        </van-field>
      </template>
      <div class="footer">
        <van-button block type="info" native-type="submit" color="#188FF9">
          提交
        </van-button>
      </div>
    </van-form>
...
export default {
  ...
  methods: {
    ...
    onConfirm(value) {
      if (this.getDateDiff(value[1].text, value[2].text)) {
        return this.$toast.fail('开始时间不能大于结束时间')
      }
      let curTime = dayjs().valueOf();
      let selectTime = dayjs(value[0] + ' ' + value[1].text + ':' + value[2].text).valueOf();
      if (selectTime < curTime) {
        return this.$toast.fail('看车时间不可早于当前时间')
      }
